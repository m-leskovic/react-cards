...
const EditCard = observer(({ store }) => {
  ...
  const showHidePin = action(() => {
    !store.showPin ? (store.showPin = true) : (store.showPin = false);
  });
    ...
    <Container fluid="md" className="card-details p-2 p-sm-3">
      <Form onSubmit={handleEditSubmit}>
        ...
        <Row className="details-inner-wrapper gap-4 gap-sm-3 pb-3 mx-sm-1">
          <Col className="num-wrapper">
            ...
            <Form.Control
              ...
              name="number"
              maxLength={19}
              value={store.currentCard.number
                .replace(/\W/gi, "")
                .replace(/(.{4})/g, "$1 ")
                .trim()}
            ...
            <Col className="pin-wrapper pe-0 pe-sm-1">
              ...
              <Row>
                <Col className="d-flex align-items-start">
                  ...
                  <Button
                    type="button"
                    className="fa-pin ms-1 ms-sm-2"
                    onClick={showHidePin}
                  >
                    <FontAwesomeIcon
                      icon={store.showPin ? faEyeSlash : faEye}
                    />
                  </Button>
                </Col>
              </Row>
        ...
        <Row className="amount-wrapper my-3 mx-sm-1">
          <Col>
            <h4 className="details-title text-end fw-bold mb-3">
              Amount on card (€):
            </h4>
            <Form.Control
              className="text-end"
